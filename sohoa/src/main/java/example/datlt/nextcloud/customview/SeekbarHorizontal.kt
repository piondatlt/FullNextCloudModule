...
class SeekbarHorizontal(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        thumbRadius = height.toFloat() / 2
        val backgroundHeight = height.toFloat() / 22 * 6
        val backgroundTop = (height.toFloat() / 2) - (backgroundHeight / 2)
        val backgroundBottom = (height.toFloat() / 2) + (backgroundHeight / 2)
        backGroundWidth = width - (thumbRadius * 2)
        backGroundRect.set(thumbRadius, backgroundTop, width - thumbRadius, backgroundBottom)
    ...
    fun setProgressChangeListener(listener: CustomSeekbarInterface) {
        this@SeekbarHorizontal.listener = listener
    }

    private fun syncThumbRectToProgress() {
        val left = (backGroundWidth / 100) * progress
        val right = left + (thumbRadius * 2)
        thumbRect.set(left, 0f, right, height.toFloat())
    }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event?.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (event.x < thumbRadius) {
                    progress = 0
                    return true
                } else if (event.x > width - thumbRadius) {
                    progress = 100
                    return true
                } else {
                    progress =
                        ((maxProgress.toFloat() / (width - 2 * thumbRadius)) * (event.x - thumbRadius)).toInt()
                }
                if (progress < 0) {
                    progress = 0
                }
